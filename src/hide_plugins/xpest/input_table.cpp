...
InputTable::InputTable(Identifier name, QObject *parent)
	: Model(name, parent)
{
    addParameter<QString>(Name(fileName), QString(), "Name of column-based text file");
    addParameter<QString>(Name(xName), QString(), "Name of x column");
    addParameter<QString>(Name(yName), QString(), "Name of y column");
    addParameter<double>(Name(xValue), 0., "Value of x to interpolate to y");
    addVariable<double>(Name(yValue), "Y value interpolated from x");
}
