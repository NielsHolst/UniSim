...
double Model::pullVariable(Identifier name) {
    return seekOneChild<PullVariable*>(name.key())->value();
}

const double* Model::pullVariablePtr(Identifier name) {
    return seekOneChild<PullVariable*>(name.key())->valuePtr();
}
