...
template<class X, class Y> double interpolate(QPair<X,Y> p1, QPair<X,Y> p2, X x);
template<class X, class Y> double interpolate(const QMap<X,Y> &xy, X x);
template<class XYZ> double interpolate(const Matrix<XYZ> &xyz, XYZ x, XYZ y) {return 0.;}
...
double interpolate(QPair<X,Y> p1, QPair<X,Y> p2, X x) {
...
double interpolate(const QMap<X,Y> &xy, X x) {
    ...
    Y lastY = xy[0];
    auto low = xy.begin();
    for (QMap<X,Y>::const_iterator high = xy.begin();
                                           high != xy.end();
                                           ++high)
    {
        if (x < high.key()) {
...
template<class XYZ> double interpolate(const Matrix<XYZ> &xyz, XYZ x, XYZ y) {
