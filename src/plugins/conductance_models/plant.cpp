    ...
    : Model(name, parent)
{
    ...
    new PullVariable("phase", &_phase, this, "Competition phase: @F Unlimited (0), @F UnderCompression (1), or @F WeightProportional (2).");
}

void Plant::initialize() {
    setParameter("initWeight", &initWeight, 2., "Initial plant weight at time zero (g per plant)");
    setParameter("A", &A, 0.03, "Coefficient in allometric relation for crown zone area: @Math {A w sup phione}");
    setParameter("phi", &phi, 0.67, "Exponent in allometric relation for crown zone area: @Math {A w sup phione}");
    setParameter("F", &F, 0.01, "Coefficient in allometric relation for plant leaf area: @Math {F w sup theta}");
    setParameter("theta", &theta, 0.9, "Exponent in allometric relation for plant leaf area: @Math {F w sup theta}");
