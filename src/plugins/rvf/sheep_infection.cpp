/* Copyright (C) 2009-2012 by Niels Holst [niels.holst@agrsci.dk] and co-authors.
** Copyrights reserved.
** Released under the terms of the GNU General Public License version 3.0 or later.
** See www.gnu.org/copyleft/gpl.html.
*/
#include "publish.h"
#include "sheep_infection.h"

using namespace UniSim;

namespace rvf {

PUBLISH(SheepInfection)

SheepInfection::SheepInfection(Identifier name, QObject *parent)
     : Model(name, parent)
{
    Input(double, mortalityRate, 0.);   // Sheep death rate due to disease per day
    Input(double, density, 0.);         // Density of infected hosts
    Output(double, mortality);          // Number died
}
void SheepInfection::reset() {
    mortality = 0.;
}
void SheepInfection::update() {
    mortality = mortalityRate*density;
}
} //namespace
